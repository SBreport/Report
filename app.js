...
function renderPreview(data) {
  ...
  h += `</div>`;
    ...
    h += `<p>🥇 <strong>${lfBest1.title}</strong> — ${formatNumber(lfBest1.views)}</p>`;
...
function buildCalendarHTML(year, month, videos, mode) {
  ...
  for (let d = 1; d <= lastDate; d++) {
    ...
    dayVideos.forEach(v => {
      ...
      const title = v.title.length > 6 ? v.title.substring(0, 6) + '…' : v.title;

      if (isPreview) {
        h += `<div class="${badgeClass} ${typeClass}" title="${v.title} — ${formatNumber(v.views)}"><span class="cal-type-tag">${typeLabel}</span> ${title}</div>`;
      } else {
        h += `<div class="${badgeClass} ${typeClass}" title="${v.title}">`;
